...
export default function JnfPreviewPage() {
  ...
  if (loading) return <Box sx={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', bgcolor: '#F4F6F9' }}><CircularProgress sx={{ color: '#0A1628' }} /></Box>;

  if (submitted) {
    return (
      <Box sx={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', bgcolor: '#F4F6F9' }}>
        <Box sx={{ textAlign: 'center', maxWidth: 500 }}>
          <Box sx={{ width: 72, height: 72, borderRadius: '50%', bgcolor: 'rgba(34,100,60,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 3 }}>
            <Typography sx={{ fontSize: '32px' }}>✓</Typography>
          </Box>
          <Typography sx={{ fontFamily: '"EB Garamond", serif', fontSize: '32px', fontWeight: 500, color: '#0A1628', mb: 1 }}>JNF Submitted Successfully</Typography>
          <Typography sx={{ fontSize: '14px', color: '#5A6478', mb: 1 }}>Your Job Notification Form has been submitted for review.</Typography>
          ...
          <Typography sx={{ fontSize: '13px', color: '#5A6478', mb: 3 }}>A confirmation email has been sent to your registered email address. The CDC team will review your submission shortly.</Typography>
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#F4F6F9', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ bgcolor: '#0A1628', color: '#FEFEFE', px: 4, py: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          <Typography sx={{ fontSize: '18px', fontWeight: 600, fontFamily: '"EB Garamond", serif' }}>JNF Preview</Typography>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button component={Link} href={`/jnf/${id}`}
            sx={{ color: '#FEFEFE', borderColor: 'rgba(255,255,255,0.3)', border: '1px solid', fontSize: '13px' }}>
            ← Edit Form
          </Button>
          {data?.status === 'draft' && (
            <Button onClick={() => setConfirmOpen(true)} variant="contained"
              sx={{ bgcolor: '#C8922A', color: '#0A1628', fontWeight: 600, '&:hover': { bgcolor: '#E8B64A' } }}>
              Submit JNF →
            </Button>
          )}
        </Box>
      ...
      <Box sx={{ flex: 1, p: { xs: 2, md: 4 }, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <Box sx={{ width: '100%', maxWidth: 960 }}>
          ...
          <Box sx={sectionBox}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography sx={sectionTitle}>Job Profile</Typography>
              <Button component={Link} href={`/jnf/${id}`} size="small" sx={{ fontSize: '12px', color: '#2A8A9E' }}>Edit</Button>
            </Box>
            {jp ? (
              <Grid container spacing={1} sx={{ mt: 1 }}>
                <FieldDisplay label="Profile Name" value={jp.profile_name} />
                <FieldDisplay label="Designation" value={jp.designation} />
                <FieldDisplay label="Place of Posting" value={jp.place_of_posting} />
                <FieldDisplay label="Work Mode" value={workModeLabel[jp.work_mode]} />
                <FieldDisplay label="Expected Hires" value={jp.expected_hires} />
                <FieldDisplay label="Min Hires" value={jp.min_hires} />
                <FieldDisplay label="Joining Month" value={jp.tentative_joining_month} />
                <FieldDisplay label="Required Skills" value={jp.required_skills} />
                {jp.job_description && (
                  <Grid item xs={12}>
                    <Typography sx={fieldLabel}>Job Description</Typography>
                    <Typography sx={{ ...fieldValue, whiteSpace: 'pre-wrap' }}>{jp.job_description}</Typography>
                  </Grid>
                )}
                <FieldDisplay label="Bond Details" value={jp.bond_details} />
              </Grid>
          ...
          <Box sx={sectionBox}>
            ...
            {ec ? (
              <Grid container spacing={1} sx={{ mt: 1 }}>
                ...
                <FieldDisplay label="HS Percentage" value={ec.hs_percentage ? `${ec.hs_percentage}%` : null} />
                  ...
                  <Grid item xs={12}>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mb: 1.5 }}>
                      {ec.programmes.filter((p: any) => p.is_selected).map((p: any, i: number) => (
                        <Chip key={i} label={p.programme_name || p.programme_code} size="small" sx={{ bgcolor: 'rgba(200,146,42,0.1)', color: '#0A1628' }} />
                      ))}
                    </Box>
                  </Grid>
                )}
          ...
          <Box sx={sectionBox}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography sx={sectionTitle}>Salary Details</Typography>
              <Button component={Link} href={`/jnf/${id}`} size="small" sx={{ fontSize: '12px', color: '#2A8A9E' }}>Edit</Button>
            </Box>
            {sal.length > 0 ? sal.map((s: any, i: number) => (
              <Box key={i} sx={{ p: 2, bgcolor: '#F4F6F9', borderRadius: 1, mb: 1.5, border: '1px solid rgba(10,22,40,0.06)' }}>
                <Typography sx={{ fontSize: '13px', fontWeight: 600, color: '#0A1628', mb: 1 }}>{s.programme?.replace('_', ' / ')}</Typography>
                <Grid container spacing={1}>
                  <FieldDisplay label="CTC (Annual)" value={formatAmount(s.ctc_annual, s.currency)} />
                  <FieldDisplay label="Base / Fixed" value={formatAmount(s.base_fixed, s.currency)} />
                  <FieldDisplay label="Monthly Take-home" value={formatAmount(s.monthly_takehome, s.currency)} />
                  <FieldDisplay label="Gross Salary" value={formatAmount(s.gross_salary, s.currency)} />
                </Grid>
              </Box>
            )) : <Typography sx={{ color: '#5A6478', fontStyle: 'italic' }}>Not filled yet</Typography>}
          </Box>
          ...
          <Box sx={sectionBox}>
            ...
            {ss.length > 0 ? ss.filter((s: any) => s.is_enabled).map((s: any, i: number) => (
              <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1, borderBottom: '1px solid rgba(10,22,40,0.06)' }}>
                <Typography sx={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '12px', color: '#C8922A', fontWeight: 600, minWidth: 24 }}>{i + 1}</Typography>
          ...
          <Box sx={sectionBox}>
            ...
            {dec ? (
              <Grid container spacing={1} sx={{ mt: 1 }}>
                ...
                <FieldDisplay label="All Declarations Agreed" value={dec.aipc_guidelines && dec.shortlisting_commitment && dec.accuracy_profile ? '✓ Yes' : '✗ Incomplete'} />
              </Grid>
            ) : <Typography sx={{ color: '#5A6478', fontStyle: 'italic' }}>Not filled yet</Typography>}
          </Box>
        </Box>
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography sx={{ color: '#5A6478' }}>Once submitted, you won't be able to edit this JNF. The CDC team will review and respond. Are you sure?</Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setConfirmOpen(false)} sx={{ color: '#5A6478' }}>Cancel</Button>
          <Button onClick={handleSubmit} variant="contained" sx={{ bgcolor: '#C8922A', color: '#0A1628', fontWeight: 600, '&:hover': { bgcolor: '#E8B64A' } }}>
            Yes, Submit JNF
          </Button>
        </DialogActions>
      </Dialog>
